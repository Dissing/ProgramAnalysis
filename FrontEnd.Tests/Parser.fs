...
let parseArithmetic1 () =
    ...
    let expected: Context<Statement list> = Ok([
        Assign(Variable "z", ArithmeticBinary (Loc (Variable "x"), Add, IntLiteral(1)))
    ])
    Assert.That(ast, Is.EqualTo(expected))
...
let parseArithmetic2 () =
    ...
    let e1 = ArithmeticBinary (Loc (Variable "c"), Add, Loc (Variable "d"))
    let e2 = ArithmeticUnary  (Negative, Loc (Variable "b"))
    let e3 = ArithmeticBinary (e2, Multiply, e1)
    let e4 = ArithmeticBinary (Loc (Variable "a"), Add, e3)
    
    let expected: Context<Statement list> = Ok([
        Assign(Variable "z", e4)
    ])
    Assert.That(ast, Is.EqualTo(expected))
...
let parseArithmetic3 () =
    ...
    let e2 = ArithmeticBinary ((Loc (Variable "x")), Multiply, (IntLiteral 2))
...
let parseIf () =
    ...
    let thenBlock = [Assign(Variable "x", IntLiteral 0)]
...
let parseIfElse () =
    ...
    let thenBlock = [Assign(Variable "x", IntLiteral 0)]
    let elseBlock = [Assign(Variable "z", IntLiteral 0)]
...
let parseWhile () =
    ...
    let loopBlock = [Assign(Variable "x", IntLiteral 0)]
...
let parseComparisons () =
    ...
    let e2 = Comparison (Loc (Variable "a"), ComparisonOperator.Lesser, Loc (Variable "b"))
...
let parseIntDecl() =
    ...
    let d1 = Allocate(VarDecl "x")
    let d2 = Allocate(VarDecl "averylongandwindynameforpuretestingpurposes")
...
let parseStructLiteral() =
    ...
    let assign = RecordAssign("foo", [("", a1); ("", a2); ("", a3)])
...
let parseStructDecl() =
    ...
    let di1 = Allocate(VarDecl "x")
    let di2 = Allocate(VarDecl "y")
    
    let ds = Allocate(RecordDecl("strct", ["x";"y";"z"]))
...
let parseFieldAccess() =
    ...
    let ast = Lexer.lex source.Content >>= Parser.parse
    let assign = Assign ((Variable "x"), ArithmeticBinary(Loc (Field("strct", "fld")), Add, IntLiteral 1))
...
let parseModuloProgram() =
    ...
    let declX = Allocate(VarDecl "x")
    let declY = Allocate(VarDecl "y")
    
    let readX = Read(Variable "x")
    let readY = Read(Variable "y")
    let condition = Comparison(Loc(Variable "x"), Greater, Loc(Variable "y"))
    let assign = Assign(Variable "x", ArithmeticBinary(Loc (Variable "x"), Subtract, Loc (Variable "y")))
    let block = [assign]
    let while_ = While(condition, block)
    let writeX = Write (Loc (Variable "x"))
